...
test('getProp', async t => {
  ...
  const cases = [
    { obj: { cake: { name: "chocolate", price: 10 } }, path: 'cake.name', expected: 'chocolate' },
    { obj: { cake: { name: "chocolate", price: 10 } }, path: 'cake.price', expected: 10 },
...
test('populateEventData', async t => {
  let result = populateEventData({ cake: { name: "{{cakeType}}", price: "{{cakePrice}}" } },
    {
      cakeType: "Cheese",
      cakePrice: 50
    });
  t.is(JSON.stringify(result), JSON.stringify({ cakeType: { S: 'Cheese' }, cakePrice: { N: '50' } }));
});
