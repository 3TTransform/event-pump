...
test.serial.skip(
    '🍎 mssqlHydrateOne log an error if input is missing',
    async t => {
        const pattern = {
            action: {
                params: {
...
test.serial.skip(
    '🍎 mssqlHydrateOne should throw an error if parameters are not supplied',
    async t => {
        const pattern = {
            action: {
                params: {
...
test.serial.skip(
    '🍎 mssqlHydrateOne should throw an error if the SQL is not valid',
    async t => {
        const pattern = {
            action: {
                params: {
        ...
        async t => {
            ...
            try {
                ...
                if (field.type == 'Xml')
                    t.is(
                        e.message,
                        `Convertion of [object Object] to ${field.type} failed`,
                    );
                else
                    t.is(
                        e.message,
                        `Convertion of ${field.value} to ${field.type} failed`,
                    );
            }
        },
    );
