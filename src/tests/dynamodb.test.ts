...
test.serial('🍎 generateUpdateQuery should return undefined if fields are either null or undefined', (t) => {
...
test.serial('🍎 generateUpdateQuery should handle an empty fields object', (t) => {
    const fields = {};
...
test.serial('🍏 scanTable should call the scan method with correct parameters', async (t) => {
...
test.serial('🍏 dynamodbTableExists should return true when the table exists', async (t) => {
    const tableName = 'myTable';
...
test.serial('🍎 dynamodbTableExists should return false when the table does not exist', async (t) => {
    const tableName = 'myTable';
...
test.serial('🍏 dynamodbHydrateOne should call dynamodbWrite method for "put" action type', async (t) => {
...
test.serial('🍏 dynamodbHydrateOne should call dynamodbUpdate method for "update" action type', async (t) => {
    dynamoStub.listTables = sinon.stub().returns( {promise: () => ({ TableNames: ['myTable'] })});
    const pattern = {
        action: {
            values: {
    ...
    const expectedResult = {
        ...
        ExpressionAttributeNames: { '#id': 'id', '#organisationName': 'organisationName' },
        ExpressionAttributeValues: {
...
test.serial('🍏 dynamodbHydrateOne should call dynamodbDelete method for "delete" action type', async (t) => {
    dynamoStub.listTables = sinon.stub().returns( {promise: () => ({ TableNames: ['myTable'] })});
    const pattern = {
        action: {
            params: params,
